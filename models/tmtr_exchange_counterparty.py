...
class TmtrExchangeOneCCounterparty(models.Model):
    ...
    def upload_parnter_ref(self, limit=1):
        partners = self.env['tmtr.exchange.1c.partner'].search([("partner_id", "=", None)], limit=limit)
        for partner in partners:
            counterparty = self.env['tmtr.exchange.1c.counterparty'].search([("partner_key", "=", partner['ref_key'])])
            if not counterparty:
                continue
            partner.update({
    ...
    def add_res_partner(self, limit=1):
        # obj = {}
